...
async function refresh(message) {
  ...
  document.getElementById("week-chip").textContent = `W${String(state.week_number).padStart(2, "0")}`;
  render();
...
function timerCard() {
  if (state.running) {
    return `<section class="timer running"><div class="timer-state">Clocked in · Goal ${state.running.number}</div><div class="clock" id="live-clock">00:00:00</div><div class="timer-title">${esc(state.running.title)}</div><button class="primary stop" id="clock-out">Clock out</button></section>`;
  }
  const options = state.goals.filter((goal) => goal.task_id).map((goal) =>
    `<option value="${goal.task_id}">${goal.number} · ${esc(goal.task_title)}${goal.is_extra_time ? " · EXTRA" : ""}</option>`).join("");
  return `<section class="timer"><div class="timer-state">Start the next block</div><div class="clock">00:00:00</div><label class="sub" for="goal-select">Goal contract</label><select id="goal-select"><option value="">Select goal…</option>${options}</select><button class="primary" id="clock-in">Clock in</button></section>`;
}

function renderNow() {
  const savingsGap = state.eligible_savings_cents - state.savings_pace_cents;
  app.innerHTML = `<div class="eyebrow">Chapter I · week ${String(state.week_number).padStart(2, "0")} of 38</div><h1>${state.days_left} days. One current action.</h1><p class="sub">Evidence freeze · May 5, 2027</p>
    ${timerCard()}
    <div class="stat-grid">
      ...
      ${metric("Self-payroll owed", dollars(state.liability_cents), "internal · unsettled")}
      ${metric("Completed capital", dollars(state.completed_capital_cents), "settled contracts")}
      ${metric("Adjusted capital", dollars(state.contract_adjusted_capital_cents), "cash − commitments − goal payroll", statusClass(state.contract_adjusted_capital_cents))}
    </div>
    <h2>Week pressure</h2><div class="list">${state.goals.filter((goal) => goal.weekly_target_minutes > 0).map(goalCard).join("")}</div>`;
  wireTimer();
}
...
function goalCard(goal, full = false) {
  ...
  return `<article class="goal"><div class="goal-head"><span class="goal-number">0${goal.number}</span><span class="pill ${goal.weekly_target_minutes ? statusTone : "extra"}">${status}</span></div>
    ...
    ${full && goal.task_id ? `<div class="goal-actions"><button class="secondary finish" data-task="${goal.task_id}" data-title="${esc(goal.task_title)}">Finish contract</button></div>` : ""}
    ${full && !goal.task_id ? `<div class="goal-actions"><button class="secondary open-task" data-goal="${goal.id}" data-deadline="${goal.deadline}">Open next contract</button></div>` : ""}</article>`;
}

function renderGoals() {
  app.innerHTML = `<div class="eyebrow">Contract ledger</div><h1>Time is debt until the work ships.</h1><p class="sub">Clock-out accrues WIP and self-payroll. Proof settles both.</p>
    <div class="stat-grid">${metric("Goal WIP", dollars(state.liability_cents))}${metric("Hourly rate", dollars(state.rate_cents), "universal internal rate")}${metric("Goal capital", dollars(state.completed_capital_cents))}${metric("Weekly labor", hours(state.goals.reduce((sum, goal) => sum + goal.week_minutes, 0)), "hours : minutes")}</div>
    <div class="section-title"><h2>All outcomes</h2><button class="secondary" id="set-rate">Set rate</button></div><div class="list">${state.goals.map((goal) => goalCard(goal, true)).join("")}</div>`;
...
function renderMoney() {
  ...
  app.innerHTML = `<div class="eyebrow">Financial book</div><h1>Actual money stays actual.</h1><p class="sub">Goal payroll is shown separately and never posted into household equity.</p>
...
function renderClose() {
  ...
  app.innerHTML = `<div class="eyebrow">One close</div><h1>Review once. Lock it. Move on.</h1><p class="sub">Week of ${esc(state.week_start)} · month ${esc(state.month.period)}</p>
    ...
    <h2>Weekly goal book</h2><div class="section">${state.week.rows.map((row) => `<div class="money-row"><span>0${row.number} · ${esc(row.outcome)}</span><span>${hours(row.minutes)}<br><span class="sub">${dollars(row.accrued_cents)}</span></span></div>`).join("")}</div>
...
function finishDialog(taskId, title) {
  openDialog(`<div class="dialog-title">Finish contract</div><p class="sub">${esc(title)}</p><form><div class="field"><label for="evidence">Proof location or receipt</label><input id="evidence" name="evidence" required autofocus></div><button class="primary" type="submit">Settle contract</button></form>`,
    (data) => act("task-complete", { task_id: Number(taskId), evidence: data.get("evidence") }, "Contract settled."));
}

function taskDialog(goalId, deadline) {
  openDialog(`<div class="dialog-title">Open next contract</div><form><div class="field"><label for="title">One finish</label><input id="title" name="title" required autofocus></div><div class="field"><label for="due">Due</label><input id="due" name="due" type="date" value="${esc(deadline)}" required></div><button class="primary" type="submit">Open contract</button></form>`,
    (data) => act("task-create", { goal_id: Number(goalId), title: data.get("title"), due_date: data.get("due") }, "Contract opened."));
}
